	...
	switch(d)
	{
		case '1':
...

			{
				...
				{
				while(fread(&p,sizeof(Astha),1,fp))
				s=1+p.sn;
				}
				fclose(fp);
			}
		...
		case '4':
			{
				...
				while(fread(&p,sizeof(Astha),1,fp))
					if(p.sn!=y)
					fwrite(&p,sizeof(Astha),1,f1);
				clrscr();
				mainscreen();
				fseek(fp,(y-1)*sizeof(Astha),SEEK_SET);
				fread(&p,sizeof(Astha),1,fp);
				print();
				edit1();
				gotoxy(25,46);
				...
				case '1':
					{
					 ...
					 fseek(fp,0,SEEK_END);
					 tsz=ftell(fp);
					 n=(int)(tsz/sizeof(Astha));
					 for(i=0;i<(n-1);i++)
					 {
						for(a=i+1;a<n;a++)
						{
						fseek(fp,i*sizeof(Astha),SEEK_SET);
						fread(&p,sizeof(Astha),1,fp);
						fseek(fp,a*sizeof(Astha),SEEK_SET);
						fread(&q,sizeof(Astha),1,fp);
						if(strcmp(p.name,q.name)>0)
							{
							fseek(fp,i*sizeof(Astha),SEEK_SET);
							fwrite(&q,sizeof(Astha),1,fp);
							fseek(fp,a*sizeof(Astha),SEEK_SET); fflush(stdin);
							fwrite(&p,sizeof(Astha),1,fp);
							}
						}
					}
					rewind(fp);
...
void mainscreen()
{
	...
	cprintf("Astha HOSPITAL");
	gotoxy(27,6);textcolor(6);
	cprintf("Rohini, Delhi");
